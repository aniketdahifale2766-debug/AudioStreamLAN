...
class MainActivity : AppCompatActivity() {
    ...
    private val permissionLauncher = registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) { permissions ->
        val audioOk = permissions[android.Manifest.permission.RECORD_AUDIO] == true || checkSelfPermission(android.Manifest.permission.RECORD_AUDIO) == android.content.pm.PackageManager.PERMISSION_GRANTED
        if (!audioOk) { status.text = "Audio capture permission is required"; return@registerForActivityResult }
        if (pendingProjection) launchProjection()
    }

    private val projectionLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        pendingProjection = false
        if (result.resultCode != Activity.RESULT_OK || result.data == null) { status.text = "Screen/audio capture permission cancelled"; return@registerForActivityResult }
    ...
    override fun onCreate(savedInstanceState: Bundle?) { super.onCreate(savedInstanceState); buildUi(); refreshState() }
    ...
    private fun buildUi() {
        ...
        stopButton = Button(this).apply {
            ...
            setOnClickListener { stopService(Intent(this@MainActivity, AudioCaptureService::class.java).apply { action = AudioCaptureService.ACTION_STOP }); status.text = "Stopping…" }
        }
        val copyButton = Button(this).apply {
            text = "Copy listener address"
            setOnClickListener {
                val value = urlText.text.toString().removePrefix("Listener address: ")
                if (value.startsWith("http://")) { (getSystemService(CLIPBOARD_SERVICE) as ClipboardManager).setPrimaryClip(ClipData.newPlainText("AudioStreamLAN", value)); status.text = "Address copied" }
            }
        }
        ...
        val content = LinearLayout(this).apply {
            ...
            addView(TextView(this@MainActivity).apply { text = "Listener: scan the QR code or open the address, tap Start Listening, then choose 80–1000 ms delay. Every browser controls its own delay."; textSize = 13f; setPadding(8, 16, 8, 8) })
        }
    ...
    private fun requestCapturePermissions() {
        ...
        pendingProjection = true; permissionLauncher.launch(permissions.toTypedArray())
    }

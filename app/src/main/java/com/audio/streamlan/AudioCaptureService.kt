...
class AudioCaptureService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        try {
            ServiceCompat.startForeground(
                this, NOTIFICATION_ID, buildNotification("Starting local audio stream…"),
        ...
        worker = Thread {
            ...
            try {
                while (!Thread.currentThread().isInterrupted && !stopping) {
                    ...
                    val count = current.read(readBuffer, 0, readBuffer.size, AudioRecord.READ_BLOCKING)
                    if (count > 0) {
                        if (mono) {
                            var out = 0; var i = 0
                            while (i + 1 < count && out + 3 < outputBuffer.size) {
                                outputBuffer[out++] = readBuffer[i]; outputBuffer[out++] = readBuffer[i + 1]
                                outputBuffer[out++] = readBuffer[i]; outputBuffer[out++] = readBuffer[i + 1]
                                i += 2
                            }
    ...
    private fun saveClientCount() = runCatching {
    ...
    private fun cleanupSafely() {
        runCatching { worker?.interrupt() }; worker = null
        runCatching { recorder?.stop() }; runCatching { recorder?.release() }; recorder = null
        runCatching { projection?.stop() }; projection = null
        runCatching { server?.stopServer() }; server = null
        runCatching { wakeLock?.let { if (it.isHeld) it.release() } }; wakeLock = null
        runCatching { getSharedPreferences("state", MODE_PRIVATE).edit().putBoolean("running", false).putString("value", "Stopped").putInt("clients", 0).apply() }
    ...
    private fun buildNotification(text: String): Notification = Notification.Builder(this, CHANNEL_ID)
        .setContentTitle("AudioStreamLAN").setContentText(text)
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) getSystemService(NotificationManager::class.java)
            .createNotificationChannel(NotificationChannel(CHANNEL_ID, "Audio streaming", NotificationManager.IMPORTANCE_LOW))
    }

    private fun saveState(running: Boolean, value: String) = runCatching {
        getSharedPreferences("state", MODE_PRIVATE).edit().putBoolean("running", running)
            .putString("value", value).putInt("clients", server?.clientCount ?: 0).apply()
    }

    override fun onDestroy() {
        if (!stopping) {
            stopping = true
            runCatching { worker?.interrupt() }; runCatching { recorder?.stop() }; runCatching { recorder?.release() }
            runCatching { projection?.stop() }; runCatching { server?.stopServer() }
            runCatching { wakeLock?.let { if (it.isHeld) it.release() } }
            worker = null; recorder = null; projection = null; server = null; wakeLock = null
        }
        super.onDestroy()
    }
